<?include("db.php");
		...
		if($_GET['action']=="order")
		{
			...
			for ($i=1;$i<=$iCount;$i++)
			{
				$name="txtid".$i;
				$order="txtOrder".$i;
				$sql="update tblProductsCat SET iOrder=".$_POST[$order]." where iID=".$_POST[$name];
	...
	if($_GET['action']=="delete"){
		$sql="delete from tblProductsCat where iID=".$_GET['id'];
		$result=@mysql_query($sql,$link);
		$Message="<b><i><font color='red'>Item Deleted!</font></i></b>";
	}
	if($_GET['action']=="add"){
	$sql="insert into tblProductsCat (sName,sContent)values('".mysql_real_escape_string($_POST['txtTitle'])."','".mysql_real_escape_string($_POST['txtContent'])."')";
	echo $sql;
	@mysql_query($sql,$link);
	$Message="<b><i><font color='red'>Changes Saved!</font></i></b>";
	}
	$sql="select * from tblProductsCat order by iOrder";
	$result=@mysql_query($sql,$link);
?>
...
<table width="100%" cellpadding="0" cellspacing="0">
	<tr>
		...
		<p><b>Manage Product Category</b></p>
		...
		<table border="0" width="432" cellspacing="4" cellpadding="4" class="ContentText">
			<tr>
				<td width="111" valign="top">Category Name</td>
		...
		<form  method=post action="products.php?action=order">
	...
	<tr>
		<td style="width: 10px; height: 2px;"><a href="products-items.php?id=<?echo $row['iID'];?>&name=<?echo $row['sName'];?>"><?echo $row['sName'];?></a></td>
		<td bgcolor="#444444" width="2" style="height: 2px"><a href="products-edit.php?id=<?echo $row['iID'];?>">Edit</a></td>
		<td bgcolor="444444" class="LeftLink" style="height: 2px;" align="right" valign="top" colspan="3"><a href="products.php?action=delete&id=<?echo $row['iID'];?>">Delete</a></td>
