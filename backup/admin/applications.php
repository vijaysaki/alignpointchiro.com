<?include("db.php");
		...
		if($_GET['action']=="order")
		{
			...
			for ($i=1;$i<=$iCount;$i++)
			{
				$name="txtid".$i;
				$order="txtOrder".$i;
				$sql="update tblApplications SET iOrder=".$_POST[$order]." where iID=".$_POST[$name];
	...
	if($_GET['action']=="delete"){
		$sql="delete from tblApplications where iID=".$_GET['id'];
		$result=@mysql_query($sql,$link);
		$Message="<b><i><font color='red'>Item Deleted!</font></i></b>";
	}
	if($_GET['action']=="add"){
	$sql="insert into tblApplications (sTitle,sContent)values('".mysql_real_escape_string($_POST['txtTitle'])."','".mysql_real_escape_string($_POST['txtContent'])."')";
	@mysql_query($sql,$link);
	$Message="<b><i><font color='red'>Changes Saved!</font></i></b>";
	}
	$sql="select * from tblApplications order by iOrder";
	$result=@mysql_query($sql,$link);
?>
...
<table width="100%" cellpadding="0" cellspacing="0">
	<tr>
		...
		<p><?echo $Message;?></p>
		<form action="applications.php?action=add" method=post>
		<table border="0" width="462" cellspacing="4" cellpadding="4" class="ContentText">
			<tr>
				<td width="120" valign="top">Application Name</td>
				<td width="314"><input type="text" name="txtTitle" size="43"></td>
			</tr>
			<tr>
				<td width="120" valign="top">Information</td>
				<td width="314"><textarea id="textarea1" rows="9" name="txtContent" cols="34"></textarea>
		...
		Edit/Update Applications<br>
		<form  method=post action="applications.php?action=order">
	...
	<tr>
		<td style="width: 10px; height: 2px;"><?echo $row['sTitle'];?></td>
		<td bgcolor="#444444" width="2" style="height: 2px"><a href="applications-edit.php?id=<?echo $row['iID'];?>">Edit</a></td>
		<td bgcolor="444444" class="LeftLink" style="height: 2px;" align="right" valign="top" colspan="3"><a href="applications.php?action=delete&id=<?echo $row['iID'];?>">Delete</a></td>
