<?include("db.php");
		...
		if($_GET['action']=="order")
		{
			...
			for ($i=1;$i<=$iCount;$i++)
			{
				$name="txtid".$i;
				$order="txtOrder".$i;
				$sql="update tblAboutUs SET iOrder=".$_POST[$order]." where iID=".$_POST[$name];
	...
	if($_GET['action']=="delete"){
		$sql="delete from tblAboutUs where iID=".$_GET['id'];
		$result=@mysql_query($sql,$link);
		$Message="<b><i><font color='red'>Item Deleted!</font></i></b>";
	}
	if($_GET['action']=="add"){
	$sql="insert into tblAboutUs (sName,sContent)values('".mysql_real_escape_string($_POST['txtTitle'])."','".mysql_real_escape_string($_POST['txtContent'])."')";
	@mysql_query($sql,$link);
	$Message="<b><i><font color='red'>Changes Saved!</font></i></b>";
	}
	$sql="select * from tblAboutUs order by iOrder";
	$result=@mysql_query($sql,$link);
?>
...
<table width="100%" cellpadding="0" cellspacing="0">
	<tr>
		...
		<br>
		<a href="about-us.php">
		<img border="0" src="images/active-link-manage-about-us.jpg" width="197" height="33" alt="Manage About Us"></a></p>
		<p>
		<a href="staff.php">
		...
		<p><b>Manage About Us</b></p>
		<p>Add About Us</p>
		<form action="about-us.php?action=add" method=post name="add">
		<table border="0" width="432" cellspacing="4" cellpadding="4" class="ContentText">
			<tr>
				<td width="111" valign="top">About Name</td>
		...
		Edit/Update About<br>
		<form  method=post action="about-us.php?action=order">
	...
	<tr>
		...
		<td bgcolor="#444444" width="2" style="height: 2px"><a href="about-us-edit.php?id=<?echo $row['iID'];?>">Edit</a></td>
		<td bgcolor="444444" class="LeftLink" style="height: 2px;" align="right" valign="top" colspan="3"><a href="about-us.php?action=delete&id=<?echo $row['iID'];?>">Delete</a></td>
