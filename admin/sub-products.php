<?include("db.php");
		...
		if($_POST['action']=="order")
		{
			...
			for ($i=1;$i<=$iCount;$i++)
			{
				$name="txtid".$i;
				$order="txtOrder".$i;
				$sql="update tblSubProducts SET iOrder=".$_POST[$order]." where iID=".$_POST[$name];
	...
	if($_GET['action']=="delete"){
		$sql="delete from tblSubProducts where iID=".$_GET['spid'];
		$result=@mysql_query($sql,$link);
		$Message="<b><i><font color='red'>Item Deleted!</font></i></b>";
	}
	if($_GET['action']=="add"){
	$sql="insert into tblSubProducts (iCatID,iProductID,sName,sContent)values(".$_GET['id'].",".$_GET['pid'].",'".mysql_real_escape_string($_POST['txtTitle'])."','".mysql_real_escape_string($_POST['txtContent'])."')";
	@mysql_query($sql,$link);
	$Message="<b><i><font color='red'>Changes Saved!</font></i></b>";
	}
	$sql="select * from tblSubProducts where iProductID=".$_GET['pid']." order by iOrder";
	$result=@mysql_query($sql,$link);
?>
...
<table width="100%" cellpadding="0" cellspacing="0">
	<tr>
		...
		<p>Add Product</p>
		<p><?echo $Message;?></p>
		<form action="sub-products.php?pid=<?echo $_GET['pid'];?>&id=<?echo $_GET['id'];?>&name=<?echo $_GET['name'];?>&action=add" method=post>
		...
		<form  method=post action="sub-products.php?pid=<?echo $_GET['pid'];?>&id=<?echo $_GET['id'];?>&name=<?echo $_GET['name'];?>&action=order">
	...
	<tr>
		...
		<td bgcolor="#444444" width="2" style="height: 2px"><a href="sub-products-edit.php?spid=<?echo $row['iID']?>&pid=<?echo $_GET['pid'];?>&id=<?echo $_GET['id']?>&name=<?echo $_GET['name'];?>">Edit</a></td>
		<td bgcolor="444444" class="LeftLink" style="height: 2px;" align="right" valign="top" colspan="3"><a href="sub-products.php?action=delete&spid=<?echo $row['iID']?>&pid=<?echo $_GET['pid'];?>&&id=<?echo $_GET['id']?>&name=<?echo $_GET['name'];?>">Delete</a></td>
