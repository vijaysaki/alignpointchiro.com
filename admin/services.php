<?include("db.php");
		...
		if($_GET['action']=="order")
		{
			...
			for ($i=1;$i<=$iCount;$i++)
			{
				$name="txtid".$i;
				$order="txtOrder".$i;
				$sql="update tblServices SET iOrder=".$_POST[$order]." where iID=".$_POST[$name];
	...
	if($_GET['action']=="delete"){
		$sql="delete from tblServices where iID=".$_GET['id'];
		$result=@mysql_query($sql,$link);
		$Message="<b><i><font color='red'>Item Deleted!</font></i></b>";
	}
	if($_GET['action']=="add"){
	$sql="insert into tblServices (sName,sContent)values('".mysql_real_escape_string($_POST['txtTitle'])."','".mysql_real_escape_string($_POST['txtContent'])."')";
	@mysql_query($sql,$link);
	$Message="<b><i><font color='red'>Changes Saved!</font></i></b>";
	}
	$sql="select * from tblServices order by iOrder";
	$result=@mysql_query($sql,$link);
?>
...
<table width="100%" cellpadding="0" cellspacing="0">
	<tr>
		...
		<form  method=post action="services.php?action=order">
